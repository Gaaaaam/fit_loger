...
- 日期：2026-09-04
- 状态：已实施。现行规则以 [docs/PRD.md](../../PRD.md) 为准，本文降为设计附录。
- 目标：做出一版可安装到真机、能长期自用的训练记录 app，并把「以后补不回来」的数据地基一次打好

## 实现偏差（相对本文原文）

- **编辑不自动完成。** 原文 3.4 写「手动改重量/次数自动转为 done」。现行规则：输入只保存，勾选圆圈才记 `done`；改已完成组保留原 `completed_at`。
- **Schema 已是 v2。** 原文目标 `version = 1`。现行 `CURRENT_VERSION = 2`，增加关注动作、每周训练天数、部位组数目标三张偏好表，不搬历史训练数据。
- **趋势页已做。** 原文 3.8 推迟趋势图与统计页。现行有趋势 Tab（关注动作、容量、PR、体重、周目标）。
- **3D 选肌与约 120 条动作已做。** 原文按约 34 条 catalog。现行人体模型点选肌群 + 动作说明。射线命中是包围盒入口：骨骼和结缔组织不挡肌群，只有头部外壳会挡；肩部在 0.12 的入口带宽内按射线对准盒子中心来选。
- **有氧/腹/核心分类已做。** 有氧仍按次数记，不按时长距离（见冰柜）。
