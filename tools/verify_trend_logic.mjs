...
// ---- P0-2 volume: weight × reps of work sets, warmup excluded ----
assert.equal(setVolume(row({ weight: 60, reps: 5 })), 300);
assert.equal(setVolume(row({ isWarmup: 1, weight: 40, reps: 8 })), null);
assert.equal(setVolume(row({ status: 'planned', weight: 60, reps: 5 })), null);
assert.equal(setVolume(row({ technique: 'drop', weight: 40, reps: 8 })), 320, 'drop sets still add volume');
assert.equal(setVolume(row({ weight: null, reps: 5 })), null);

const volumeCurve = buildVolumeSeries([
  row({ date: '2026-08-10', weight: 60, reps: 5 }),
  row({ date: '2026-08-10', weight: 60, reps: 5 }),
  row({ date: '2026-08-10', weight: 40, reps: 8, isWarmup: 1 }),
  row({ date: '2026-08-17', weight: 62.5, reps: 4 }),
  row({ date: '2026-08-24', weight: 50, reps: 8, status: 'planned' })
]);
...
const digestWithVolume = buildTrendDigest([
  row({ date: '2026-08-10', weight: 60, reps: 5 }),
  row({ date: '2026-08-17', weight: 62.5, reps: 4 })
], [], today, '12', prefs({ watchedExerciseIds: ['chest_bb_bench'] }));
assert.equal(digestWithVolume.volumePoints.length, 2);
assert.equal(digestWithVolume.volumePoints[0].value, 300);

// ---- P0-3 PR board: heaviest weight and best reps at that weight, work sets only ----
const prRows = [
  row({ date: '2026-08-03', exerciseId: 'chest_bb_bench', exerciseName: '杠铃卧推', weight: 60, reps: 6 }),
  row({ date: '2026-08-10', exerciseId: 'chest_bb_bench', exerciseName: '杠铃卧推', weight: 70, reps: 3 }),
  row({ date: '2026-08-17', exerciseId: 'chest_bb_bench', exerciseName: '杠铃卧推', weight: 70, reps: 5 }),
  row({ date: '2026-08-10', exerciseId: 'chest_bb_bench', exerciseName: '杠铃卧推', weight: 40, reps: 10, isWarmup: 1 }),
  row({ date: '2026-08-03', exerciseId: 'leg_squat', exerciseName: '杠铃深蹲', partKey: 'leg', movementPattern: 'squat', weight: 100, reps: 5 })
];
const board = buildPrBoard(prRows);
assert.equal(board.length, 2);
assert.equal(board[0].exerciseId, 'chest_bb_bench');
assert.equal(board[0].maxWeight, 70);
assert.equal(board[0].repsAtWeight, 5);
assert.equal(board[0].weightDate, '2026-08-10');
assert.equal(board[0].repsDate, '2026-08-17');
...
assert.equal(digestWithPr.prBoard[0].maxWeight, 70);

// ---- P0-5 e1RM: null RIR still estimates; explicit RIR>1 does not ----
assert.equal(isE1rmEligible(row({ rir: null })), false, 'strict eligibility still requires RIR');
assert.equal(isE1rmEstimated(row({ rir: null, reps: 5 })), true);
assert.equal(isE1rmEstimated(row({ rir: 3, reps: 5 })), false, 'user said they had reps in reserve');
assert.equal(isE1rmEstimated(row({ rir: null, reps: 15 })), false, 'high-rep sets are not a 1RM estimate');
assert.equal(isE1rmCountable(row({ rir: null, reps: 5 })), true);
assert.equal(isE1rmCountable(row({ rir: 3, reps: 5 })), false);

const noRirCompound = [
  row({ date: '2026-08-10', weight: 60, reps: 5, rir: null }),
  row({ date: '2026-08-17', weight: 62.5, reps: 5, rir: null })
];
const estimatedSeries = buildLiftSeries(noRirCompound, compoundPick);
assert.equal(estimatedSeries.metric, 'e1rm');
assert.equal(estimatedSeries.points.length, 2);
...
assert.equal(estimatedDetail.insufficientData, false);
