"""Build the mobile muscle atlas from actual BodyParts3D/Z-Anatomy meshes.
Run: python tools/anatomy/build_muscle_glb.py --source-dir .work/anatomy
Dependencies: numpy, scipy, fast-simplification==0.1.9. See ATTRIBUTION.md.
"""
import argparse
import hashlib
import json
...
def load_glb(path):
    ...
    for node in doc['nodes']:
        ...
        for p in doc['meshes'][node['mesh']]['primitives']:
            meshes.append((node['name'],accessor(p['attributes']['POSITION']),accessor(p['indices']).reshape(-1,3)))
...
def head_shell(points):
    # A quiet source-derived head silhouette, rather than an exposed skull.
...
def build(source_dir,output):
    ...
    def transform(v):
        return np.column_stack(((v[:,0]-center_x)*scale,(v[:,2]-lo[2])*scale,-(v[:,1]-center_y)*scale)).astype('<f4')
    groups=defaultdict(list); manifest=[]
    def add(group,name,v,f,ratio):
        if len(f)>500:
            v,f=fast_simplification.simplify(v.astype(np.float64),f.astype(np.int32),target_count=max(250,int(len(f)*ratio)),agg=5)
        n=normals_for(v,f)
        groups[group].append((name,v,n,f)); manifest.append({'structure':name,'node':group})
    for name,v,f in muscles:
        key=region_for(name)
        if key and 'tendon' not in name: group='pick_'+key+('_L' if 'left' in name else '_R')
        elif context_surface(name): group='base_connective'
        else: continue
        add(group,name,transform(v),f,.32 if key else .18)
    head=[]
    for name,v,f in bones:
        v=transform(v)
        if v[:,1].max()>1.58: head.append(v)
        else: add('base_skeleton',name,v,f,.13)
    v,f=head_shell(np.concatenate(head)); add('base_head','Source cranium silhouette (convex shell)',v,f,1)
    ...
    print(f'{len(groups)} draw groups, {sum(len(c[3]) for cs in groups.values() for c in cs):,} triangles, {output.stat().st_size/1024/1024:.1f} MiB')
